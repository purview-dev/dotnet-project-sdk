...
{
  [Test]
  public async Task EnabledAgentFolderInPackage_ExposesProjectAgentFolderInProjectTree(CancellationToken cancellationToken)
  {
    ...
    var expectedPath = Path.GetFullPath(
      Path.Combine(Path.GetDirectoryName(sdkProjectPath)!, "..", "..", "ProjectAgent", "skills", "sdk-configuration-reference", "SKILL.md")
    );
  ...
  [Test]
  public async Task EnabledAgentFolderInPackage_PacksProjectAgentSkillsIntoNuGetPackage(CancellationToken cancellationToken)
  {
    // Arrange
    await using var h = await ProjectHarness.CreateAsync(
      "PackableProject",
      extraProps:
        "<IsPackable>true</IsPackable><EnabledAgentFolderInPackage>true</EnabledAgentFolderInPackage>",
    ...
    await Assert.That(zip.Entries.Any(entry => entry.FullName == "agents/skills/observability/SKILL.md")).IsTrue();
  }

  [Test]
  public async Task EnabledAgentFolderInPackage_ErrorsWhenProjectAgentFolderIsMissing(CancellationToken cancellationToken)
  {
    // Arrange
    await using var h = await ProjectHarness.CreateAsync(
      "PackableProject",
    ...
    await Assert.That(stdOut + stdErr).Contains("ProjectAgent skills folder was not found");
  }
